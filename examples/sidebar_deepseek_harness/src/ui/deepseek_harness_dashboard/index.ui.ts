...
import {
  inspectDeepSeekHarnessRuntime,
  installDeepSeekHarnessRuntime,
  readDeepSeekHarnessWebFailure,
...
type RuntimeAction = "" | "initialize" | "update";

function clampProgress(value: number): number {
  return Math.max(0, Math.min(100, Math.round(value)));
}

function appendInstallationOutput(current: string, incoming: string): string {
  const combined = current ? `${current}\n${incoming}` : incoming;
  return combined.slice(-12000);
}

function formatVisibleInstallationOutput(output: string): string {
...
function formatStartupFailure(result: DeepSeekHarnessWebServerResult): string {
...
function formatInstallFailure(result: DeepSeekHarnessRuntimeInstallResult): string {
  if (result.diagnostic !== undefined && result.diagnostic.trim()) {
    return `${result.message}\n\n${result.diagnostic.trim()}`;
  }
  return result.message;
...
function formatInstallExecution(result: DeepSeekHarnessRuntimeInstallResult): string {
...
export default function Screen(ctx: ComposeDslContext): ComposeNode {
  ...
  const [runtimeAction, setRuntimeAction] = ctx.useState<RuntimeAction>("runtimeAction", "");
  ...
  async function startInstalledRuntime(forceRestart: boolean): Promise<void> {
    setLoading(true);
    setRuntimeAction("");
    setServerUrl("");
    ...
    setProgress(40);
    try {
      const result = await startDeepSeekHarnessWebServer({
        forceRestart,
  ...
  async function inspectAndRoute(): Promise<void> {
    setLoading(true);
    setRuntimeAction("");
    setServerUrl("");
    setPageLoading(false);
    ...
    setProgress(5);
    try {
      const inspection = await inspectDeepSeekHarnessRuntime({
        onProgress: (event: DeepSeekHarnessWebServerProgressEvent) => {
          setStatusText(event.message);
          setProgress(clampProgress(event.progress));
        },
  ...
  async function installAndStart(): Promise<void> {
    ...
    setProgress(12);
    try {
      const result = await installDeepSeekHarnessRuntime({
        onProgress: (event: DeepSeekHarnessWebServerProgressEvent) => {
          setStatusText(event.message);
          setProgress(clampProgress(event.progress));
  ...
  const actionButtons: ComposeNode[] = [];
  ...
  const versionSummary: ComposeNode[] = [];
  ...
  const progressSummary: ComposeNode[] = [];
  if (showProgress) {
        ...
        [
          UI.WebView({
            ...
            onReceivedError: async () => {
              setPageLoading(false);
              setServerUrl("");
              setErrorText(await readDeepSeekHarnessWebFailure());
            },
          }),
